...
const WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbxc-pLLUoiYVf-NJTH_f7BRz5jw7HQDMupoifr0B7zHUKZTA2iuoqaUa1wLLMQvOlCP/exec';
...
async function submitWithRetry(data, retryCount) {
  try {
    ...
    console.log('Form submitted successfully to Google Sheets (no-cors mode)', response.type);
...
export const mapFormDataToWorksheet = (formData, formType) => {
  ...
  switch (formType) {
    ...
    case 'floridaSalesTaxGuide':
    case 'miamiTaxServices':
    case 'orlandoTaxServices':
    case 'tampaTaxServices':
    case 'general':
    default:
      return {
